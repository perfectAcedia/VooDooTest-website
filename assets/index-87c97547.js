    ...
    <button id="trash" name="trash" class="absolute top-0 right-0 hover:scale-110 duration-300">
        <img src="trash-icon.svg" alt="trash icon" />
                  ...
                  <div class="mt-[10px] flex justify-between">
                    <h3 class="w-3/4 lg:w-full line-clamp-1 font-bold text-sm">${u.title}</h3>
            ...
            `,f.appendChild(L);const y=document.querySelector(`[data-id='${u.id}']`);y.addEventListener("click",()=>{const d=n.find(b=>b.id===+y.dataset.id);if(!g.has(d.id)){g.set(d.id,d);const b=g.get(d.id);C(b),R(d),A(d),k(d,y),T.innerHTML=a(+T.innerHTML+ +d.variants[0].price)}y.setAttribute("disabled","")})})}function i(l){const s=document.getElementById("links");s.innerHTML="",j(l).forEach(f=>{const h=c(f);s.appendChild(h)})}function c(l){const s=document.createElement("li");return s.classList.add("cursor-pointer","text-lg","font-normal","border","rounded-full","w-[39px]","h-[39px]","flex","justify-center","items-center","hover:scale-110","duration-300"),s.textContent=l,t===l&&(s.classList.add("text-white","bg-black"),s.setAttribute("id","active")),s.addEventListener("click",()=>{t=l,o(n,r,t),i(t),s.classList.add("text-white","bg-black"),s.setAttribute("id","active"),window.scrollTo({top:0,behavior:"smooth"})}),s}o(n,r,t),i(t)}function W(e){g.delete(+e)}const K=document.getElementById("cartButton"),z=document.getElementById("arrowDown"),q=document.getElementById("arrowIcon"),m=document.getElementById("sideMenu"),U=document.getElementById("closeButton"),M=document.getElementById("important"),E=document.getElementById("importantInfo");let w=!1;S(window.innerWidth);N();K.addEventListener("click",()=>{m.classList.remove("translate-x-full"),m.classList.add("translate-x-0"),window.innerWidth>=1024?m.style.width="25%":m.style.width="100%"});z.addEventListener("click",()=>{w?(M.style.bottom="0",E.classList.remove("flex"),E.classList.add("hidden"),q.style.transform="rotate(0)",w=!1):(M.style.bottom="-65px",E.classList.remove("hidden"),E.classList.add("flex"),q.style.transform="rotate(180deg)",w=!0)});U.addEventListener("click",()=>{m.classList.remove("translate-x-0"),m.classList.add("translate-x-full")});window.addEventListener("resize",e=>{const n=e.currentTarget.innerWidth;S(n)});
